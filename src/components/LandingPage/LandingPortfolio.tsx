...
function Portfolio() {
  return (
    <div className="p-6">
      <h1 className="text-center text-2xl font-semibold mb-6 text-customGrayText dark:text-white">Portfolio</h1>
        ...
        {projects.map((project, index) => (
          <div key={index} className="relative group w-full h-96 bg-cover bg-center rounded-lg overflow-hidden" style={{ backgroundImage: `url(${project.image})` }}>
            ...
            <div className="absolute inset-0 bg-customGray bg-opacity-50 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center transition-opacity duration-300 p-4">
              ...
              <a
                href={project.link}
                className="inline-block text-center text-customGrayText dark:text-white border border-white py-1 px-3 rounded hover:bg-white hover:text-black transition duration-300"
              >
                Visit Project
              </a>
            </div>
